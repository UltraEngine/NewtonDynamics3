...
{
	if (m_options.m_option0) angle = dClamp (angle, m_minAngle, m_maxAngle);
	if (dAbs (angle - m_targetAngle.GetAngle()) > dFloat (1.0e-3f)) {
		NewtonBodySetSleepState(m_body0, 0);
		m_targetAngle.SetAngle(angle);
	} 
}
...
{
	if (m_maxTorque == dFloat(0)) return dCustomHinge::SubmitAngularRow(matrix0, matrix1, timestep);
	...
	if (m_options.m_option0 && angle > GetMaxAngularLimit()) {
		NewtonUserJointSetRowMaximumFriction(m_joint, m_maxTorque);
	} else if (m_options.m_option0 && angle < GetMinAngularLimit()) {
